...
def create_database_link():
    ...
    global every_year
    global conn, cursor
    conn, cursor = opendb('language_voice_diction_chinese_jiangfei/PlanDatabase.sqlite')
...
def jiangfei(request):
    ...
    NOW_YMD = getnowtime()
    if os.path.exists('language_voice_diction_chinese_jiangfei/4web_restudy/'+NOW_YMD) and not os.path.exists('language_voice_diction_chinese_jiangfei/4web_restudy/common_info'):
        my_dict['showinfo'] = '已复习'
        my_dict['con'] = '已复习'
        my_dict['env'] = '已复习'
        my_dict['ext'] = '已复习'
        print('已复习')
        return render(request, 'jiangfei.html', my_dict)
    elif os.path.exists('language_voice_diction_chinese_jiangfei/4web_restudy/common_info'):
        every_year_info = loadffile('language_voice_diction_chinese_jiangfei/4web_restudy/every_year_info')
        every_month_info = loadffile('language_voice_diction_chinese_jiangfei/4web_restudy/every_month_info')
        every_week_info = loadffile('language_voice_diction_chinese_jiangfei/4web_restudy/every_week_info')
        common_info = loadffile('language_voice_diction_chinese_jiangfei/4web_restudy/common_info')
        ...
        dump2file('language_voice_diction_chinese_jiangfei/4web_restudy/every_year_info', every_year_info)
        every_month_info = every_month.find('Day', month_day)
        dump2file('language_voice_diction_chinese_jiangfei/4web_restudy/every_month_info', every_month_info)
        every_week_info = every_week.find('Day', week_day)
        dump2file('language_voice_diction_chinese_jiangfei/4web_restudy/every_week_info', every_week_info)
        common_info = common.find('Ymd', ymd)
        dump2file('language_voice_diction_chinese_jiangfei/4web_restudy/common_info', common_info)
        close_database_link()
...
#3、直到，查完事，结束并删除dump文件，生成已复习
        ...
        os.remove('language_voice_diction_chinese_jiangfei/4web_restudy/every_year_info')
        os.remove('language_voice_diction_chinese_jiangfei/4web_restudy/every_month_info')
        os.remove('language_voice_diction_chinese_jiangfei/4web_restudy/every_week_info')
        os.remove('language_voice_diction_chinese_jiangfei/4web_restudy/common_info')
        write2file('language_voice_diction_chinese_jiangfei/4web_restudy/'+NOW_YMD, '复习完成')
        return HttpResponseRedirect('/jiangfei')

    #context的'hello'对应模板html的变量{{ hello }}
    return render(request, 'jiangfei.html', my_dict)
    

def accept_cmd_jiangfei(request):
    global my_dict
    global common
    global every_week
    global every_month
